...
ensure_rule 4 -i "${EXT_IF}" -p tcp -m conntrack --ctorigdstport "${MYSQL_PORT}" -j ACCEPT
ensure_rule 5 -i "${EXT_IF}" -p tcp -m conntrack --ctorigdstport "${MONGO_PORT}" -j ACCEPT
...
echo "Docker firewall active on ${EXT_IF}: PostgreSQL ${POSTGRES_PORT}/tcp, Redis TLS ${REDIS_TLS_PORT}/tcp, MySQL TLS ${MYSQL_PORT}/tcp, and MongoDB TLS ${MONGO_PORT}/tcp allowed; other new external Docker-forwarded connections dropped."
