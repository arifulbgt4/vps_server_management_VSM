...
export default function MediaManager() {
  ...
  const apiExample = `// .env.local\nMEDIA_BASE_URL=${apiBaseUrl}\nMEDIA_API_KEY=ms_live_...\n\n// app/api/media/route.ts\nimport { NextResponse } from "next/server";\n\nconst BASE_URL = process.env.MEDIA_BASE_URL!;\nconst API_KEY = process.env.MEDIA_API_KEY!;\n\nfunction mediaHeaders() {\n  return { Authorization: \`Bearer \${API_KEY}\` };\n}\n\n// GET /api/media?mode=storage\n// GET /api/media\n// GET /api/media?fileId=<FILE_ID>\n// GET /api/media?fileId=<FILE_ID>&mode=content\nexport async function GET(request: Request) {\n  const { searchParams } = new URL(request.url);\n  const mode = searchParams.get("mode");\n  const fileId = searchParams.get("fileId");\n\n  let target = \`\${BASE_URL}/api/v1/files\`;\n\n  if (mode === "storage") {\n    target = \`\${BASE_URL}/api/v1/storage\`;\n  } else if (fileId) {\n    target = \`\${BASE_URL}/api/v1/files/\${encodeURIComponent(fileId)}\`;\n    if (mode === "content") target += "/content";\n  }\n\n  const response = await fetch(target, {\n    headers: mediaHeaders(),\n    cache: "no-store",\n  });\n\n  if (mode === "content") {\n    return new Response(response.body, {\n      status: response.status,\n      headers: {\n        "content-type": response.headers.get("content-type") || "application/octet-stream",\n        "content-disposition": response.headers.get("content-disposition") || "inline",\n      },\n    });\n  }\n\n  const data = await response.json();\n  return NextResponse.json(data, { status: response.status });\n}\n\n// POST /api/media\n// Body: FormData { file, visibility }\nexport async function POST(request: Request) {\n  const formData = await request.formData();\n\n  const response = await fetch(\`\${BASE_URL}/api/v1/files\`, {\n    method: "POST",\n    headers: mediaHeaders(),\n    body: formData,\n  });\n\n  const data = await response.json();\n  return NextResponse.json(data, { status: response.status });\n}\n\n// DELETE /api/media?fileId=<FILE_ID>\nexport async function DELETE(request: Request) {\n  const { searchParams } = new URL(request.url);\n  const fileId = searchParams.get("fileId");\n\n  if (!fileId) {\n    return NextResponse.json({ error: "fileId is required" }, { status: 400 });\n  }\n\n  const response = await fetch(\n    \`\${BASE_URL}/api/v1/files/\${encodeURIComponent(fileId)}\`,\n    { method: "DELETE", headers: mediaHeaders() },\n  );\n\n  const data = await response.json();\n  return NextResponse.json(data, { status: response.status });\n}\n\n// Client examples\n// fetch("/api/media?mode=storage")\n// fetch("/api/media")\n// fetch("/api/media", { method: "POST", body: formData })\n// fetch("/api/media?fileId=<FILE_ID>&mode=content")\n// fetch("/api/media?fileId=<FILE_ID>", { method: "DELETE" })`;
      ...
      <section className={styles.shell}>
        ...
        <section className={styles.examplePanel}>
          <div className={styles.exampleHeader}>
            <div>
              <h2>Next.js API example</h2>
              <p>Keep the media user API key server-side in <code>.env.local</code>. The Route Handler proxies storage, upload, list, binary download and hard-delete requests.</p>
            </div>
