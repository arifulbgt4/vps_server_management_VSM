...
export default function MongoDBManager() {
      ...
      if (data.password) {
        ...
        setMessage(
          data.credential_stored === false
            ? `Password changed for ${data.user}@${data.database}, but the encrypted vault update failed. Copy it now.`
            : `Credential generated for ${data.user}@${data.database} and stored encrypted.`,
        );
      } else {
        setCredential(null);
        setShowCredential(false);
  ...
  async function toggleConnection(databaseName: string, userName: string) {
    ...
    try {
      ...
      setMessage(error instanceof Error ? error.message : "Unable to reveal MongoDB URL");
    } finally {
      setBusy(false);
    }
  }
      ...
      <div className={styles.shell}>
        <div className={styles.topbar}>
          <div>
            ...
            <h1>MongoDB</h1>
            <p>Private replica-set database and per-database user management.</p>
          </div>
        ...
        {credential && (
          <section className={styles.secret}>
            <div className={styles.secretHeader}>
              <div>
                <strong>MongoDB credential</strong>
                <p>Hidden by default. The generated password is encrypted in Platform Admin.</p>
              </div>
            ...
            {showCredential && (
              <div className={styles.secretBody}>
                ...
                {credential.connection?.url && (
                  <div className={styles.secretRow}>
                    <span>Private MongoDB URL</span><code>{credential.connection.url}</code>
        ...
        <section className={styles.actions}>
          <form className={styles.card} onSubmit={createPair}>
            <h2>Create DB + user</h2>
            <p>Creates a managed database and a user with readWrite only on that database.</p>
          ...
          <form className={styles.card} onSubmit={rotate}>
            ...
            <p>Updates the MongoDB user and replaces its encrypted credential-vault copy.</p>
          ...
          <form className={styles.card} onSubmit={remove}>
            ...
            <p>For managed databases, deleting the scoped user at the same time is recommended.</p>
        ...
        <section className={styles.grid}>
                  ...
                  {resources.databases.map((item) => (
                    <tr key={item.name}>
                      ...
                      <td className={styles.connectionCell}>
                        {item.users.length === 0 ? <span className={styles.muted}>No managed user</span> : item.users.map((assignedUser) => {
                          const key = `${item.name}:${assignedUser}`;
                          const revealed = revealedConnections[key];
                          const userInfo = resources.users.find((entry) => entry.database === item.name && entry.name === assignedUser);
                          return (
                            <div key={key} className={styles.inlineSecret}>
                              <code>{assignedUser}</code>
                              {userInfo?.credential_available ? (
          ...
          <div className={styles.tableCard}>
            ...
            <div className={styles.tableWrap}>
              <table>
                <thead><tr><th>User</th><th>Auth DB</th><th>Roles</th><th>Credential</th></tr></thead>
                <tbody>
                  {resources.users.map((item) => (
                    <tr key={`${item.database}:${item.name}`}>
                      <td><code>{item.name}</code></td>
                      <td>{item.database}</td>
